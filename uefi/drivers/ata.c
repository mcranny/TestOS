...
/* Dedicated IDE data disk (see run-uefi.sh isa-ide); TFS uses the whole device. */
#define ATA_TFS_BASE_LBA       0U
#define ATA_MIN_SECTORS        2048U /* 1 MiB minimum */
...
static int ata_wait_ready(void)
{
    ...
    while (timeout-- > 0) {
        ...
        if ((status & ATA_STATUS_BSY) == 0) {
            if ((status & ATA_STATUS_ERR) != 0 || (status & ATA_STATUS_DF) != 0) return 0;
            return 1;
        }
    }
    return 0;
...
static int ata_wait_drq(void)
{
    ...
    while (timeout-- > 0) {
        status = inb(ATA_PRIMARY_STATUS);
        if ((status & ATA_STATUS_BSY) != 0) continue;
        if ((status & ATA_STATUS_ERR) != 0 || (status & ATA_STATUS_DF) != 0) return 0;
        if ((status & ATA_STATUS_DRQ) != 0) return 1;
    }
    return 0;
}
...
static int ata_read_sectors(block_device_t *device, uint32_t lba, uint32_t count, void *buffer)
{
    uint16_t *out = (uint16_t *)buffer;
    uint32_t sector, word;
    uint32_t abs;
    (void)device;
    if (!ata_present || count == 0 || count > 255U) return 0;
    for (sector = 0; sector < count; sector++) {
        abs = ATA_TFS_BASE_LBA + lba + sector;
        if (!ata_wait_ready()) return 0;
        ata_select_lba(abs, 1);
...
static int ata_write_sectors(block_device_t *device, uint32_t lba, uint32_t count, const void *buffer)
{
    ...
    for (sector = 0; sector < count; sector++) {
        ...
        if (!ata_wait_drq()) return 0;
        for (word = 0; word < (ATA_SECTOR_SIZE / 2U); word++) {
            outw(ATA_PRIMARY_DATA, in[sector * (ATA_SECTOR_SIZE / 2U) + word]);
        }
...
static int ata_identify(uint32_t *sector_count_out)
{
    ...
    if (!ata_wait_ready()) return 0;
    if (inb(ATA_PRIMARY_LBA_MID) != 0 || inb(ATA_PRIMARY_LBA_HI) != 0) return 0;
    if (!ata_wait_drq()) return 0;
    for (word = 0; word < 256U; word++) {
        identify_buffer[word] = inw(ATA_PRIMARY_DATA);
    }
    sectors = ((uint32_t)identify_buffer[61] << 16) | (uint32_t)identify_buffer[60];
...
void ata_initialize(void)
{
    ...
    if (!ata_identify(&sectors)) {
        log_warn("ATA: no disk");
        return;
    }
    if (sectors < ATA_MIN_SECTORS) {
        log_warn("ATA: disk too small for TFS");
        return;
    }
    ata_device.name = "hd0";
    ...
    if (!block_register(&ata_device)) {
        log_error("ATA: register failed");
        ata_present = 0;
        return;
    }
    log_info("ATA: hd0 TFS region ready");
}
