...
int pci_decode_bar(device_t *device, uint8_t bar_index, pci_bar_info_t *out)
{
    ...
    if (out->is_io) {
        out->phys_addr = (uint64_t)(raw & ~0x3U);
    } else {
        type_bits = (raw >> 1) & 0x3U;
        out->is_64bit = (type_bits == 0x2U) ? 1 : 0;
        out->phys_addr = (uint64_t)(raw & ~0xFU);
        if (out->is_64bit && bar_index < 5U) {
            high = pci_read_bar(device, (uint8_t)(bar_index + 1U));
            out->phys_addr |= ((uint64_t)high << 32);
        }
    }
...
static void pci_register_function(uint8_t bus, uint8_t device, uint8_t function)
{
    ...
    if (dev == NULL) {
        log_warn("PCI: Failed to allocate device entry");
        return;
    }

    name = (char *)kmalloc(16);
    if (name == NULL) {
        log_warn("PCI: Failed to allocate device name");
        kfree(dev);
        return;
    }
    ...
    if (!device_register(dev)) {
        log_warn("PCI: Failed to register device");
        kfree(name);
        kfree(dev);
        return;
    }
