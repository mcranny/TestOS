...
int tfs_check(void)
{
    ...
    if (!tfs_mounted)
    {
        log_error("tfs_check: not mounted");
        return -1;
    }
    ...
    for (index = 0; index < tfs_sb.inode_count; index++)
    {
        if (!tfs_load_inode(index, &inode))
        {
            log_error("tfs_check: inode read failed");
            return -1;
        }
        ...
        if (inode.type == (uint32_t)FS_ENTRY_DIRECTORY)
        {
            ...
            {
                log_error("tfs_check: directory has data blocks");
                errors++;
            }
            continue;
        }
        ...
        if (inode.type != (uint32_t)FS_ENTRY_FILE)
        {
            log_error("tfs_check: unknown inode type");
            errors++;
            continue;
        }
        ...
        if (inode.block_count == 0)
        {
            ...
            {
                log_error("tfs_check: empty file metadata mismatch");
                errors++;
            }
            continue;
        }
        ...
        {
            log_error("tfs_check: start_block below data region");
            errors++;
            continue;
        }
        ...
        {
            log_error("tfs_check: blocks out of range");
            errors++;
            continue;
        }
        ...
        {
            log_error("tfs_check: size exceeds allocation");
            errors++;
        }

        {
            ...
            for (offset = 0; offset < inode.block_count; offset++)
            {
                ...
                {
                    log_error("tfs_check: cross-linked block");
                    errors++;
                }
    ...
    for (bit = 0; bit < total_data; bit++)
    {
        ...
        {
            log_error("tfs_check: leaked block");
            errors++;
        }

        if (!on_disk && expected_bit)
        {
            log_error("tfs_check: allocated block not marked");
            errors++;
        }
    }
    ...
    if (errors == 0)
    {
        log_info("tfs_check: filesystem consistent");
        return 0;
    }

    log_hex64("tfs_check errors=", errors);
    return -(int)errors;
}
