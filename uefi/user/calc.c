...
static int ptr_in_user_stack(const void *pointer)
{
    ...
    /* User image (code/rodata/bss) or user stack (UEFI layout). */
    if (address >= 0x0000000000400000ULL && address < 0x0000000000410000ULL) {
        return 1;
    }
    if (address >= 0x0000000000700000ULL && address < 0x0000000000800000ULL) {
        return 1;
    }
    return 0;
}
